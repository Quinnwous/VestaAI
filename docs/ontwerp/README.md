...
| Bestand | Voor | Status |
|---|---|---|
| `kit.css` | Tokens + primitives (topbar, filterbar, dropdown/popover, segmented, chips, pills, range-slider, switch, StatTile, ChartCard, tooltip, modal, staten). Modal-sluier dimt zonder blur (item 0.1) | klaar |
| `kit.js` | Dezelfde primitives als gedrag, opmaak (`euro`/`procent`/`dagen`/`datum`), URL-state, taxonomie woningtypes, team, synthetische datagenerator, het echte i4-logo. `topbar({ actief })` is plat: zes pillen Overzicht · Woningdossier · Marktanalyse · Transacties · Concurrentie · Verkoopkaart, geen subnav (item 0.1) | klaar |
| `marktanalyse.html` | Item 6.1 (+ 6.4 kwartaalbericht) — rood accentstreepje op de kwartaalbericht-titel zodra het klaar is | klaar (0.1-review AKKOORD, 17 sep) |
