  ...
  try {
    if (doc.anthropic_file_id) {
      ...
      if (storageError || !fileData) {
        meldFout('documenten/chat:storage', storageError, { documentId: document_id })
    ...
    const ref = meldFout('documenten/chat', error, { documentId: document_id })
    return NextResponse.json({ error: 'Vraag verwerken mislukt. Probeer het opnieuw.', ref }, { status: 500 })
  }
}
